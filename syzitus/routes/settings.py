...
def settings_profile_post():
    ...
    if request.values.get("bio") is not None:
        ...
        #seo profile spam
        sus_tags=BeautifulSoup(bio_html, features="html.parser").find('a')
        if g.timestamp-g.user.created_utc < 60*60*24*2 and not g.user.post_count and not g.user.comment_count and BeautifulSoup(bio_html, features="html.parser").find('a'):
