...
TITLE_DATA={
    ...
    25: {'color': 'aa8855',
        ...
        'expr': "db.query(Board).filter(Board.creator_id==user.id, stored_subscriber_count>=100).count()",
        'kind': 1,
        'text': ', Guildsmith'},
    26: {'color': 'aa8855',
        'description': 'A Guild you created grows past 1,000 members.',
        'expr': "db.query(Board).filter(Board.creator_id==user.id, stored_subscriber_count>=1000).count()",
        'kind': 1,
        'text': ', Guildmaster'},
    27: {'color': 'aa8855',
